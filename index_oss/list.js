...
if (typeof S3_REGION != 'undefined') {
  var BUCKET_URL =
    ...
    '.' +
    S3_REGION +
    '.aliyuncs.com' // e.g. just 's3' for us-east-1 region
...
function prepareTable (info) {
  ...
  jQuery.each(files, function (idx, item) {
    ...
    if (item.Type === 'directory') {
      ...
      if (new RegExp('.*.md').test(item.Key)) {
        item.href =
          'http://chinese-quartet.org/#/markdown-previewer?url=' +
          BUCKET_WEBSITE_URL +
          '/' +
          encodePath(item.Key)
        item.target = '_blank'
    ...
    if (
      !EXCLUDE_FILE.some(function (exclude) {
        return testExcludeFilter(exclude, item.Key)
      })
    )
      content.push(row + '\n')
  })
...
function renderRow (item, cols) {
  ...
  if (item.target) {
    row +=
      '<a href="' + item.href + '" ' + 'target="' + item.target + '">' + item.keyText + '</a>'
  } else {
